...
<figure>
  <img
    src="/static/images/agri/airlift-pump-design.png"
    alt="Airlift pump design"
  />
  <figcaption>
    Desain airlift pump untuk sistem bioflok sebagai alat bantu sirkulasi air,
    suplai oksigen, dan pergerakan flok di dalam kolam.
  </figcaption>
</figure>
...
```mermaid
flowchart TD
    A[Udara dari kompresor] --> B[Pipa injeksi udara]
    B --> C[Diffuser di dasar lift tube]
    C --> D[Gelembung udara terbentuk]
    D --> E[Massa jenis campuran air-udara menurun]
    E --> F[Air terdorong naik melalui lift tube]
    F --> G[Air keluar di bagian atas]
    G --> H[Oksigenasi dan degassing terjadi]

    classDef air fill:#d9f0ff,stroke:#0077b6,color:#003049;
    classDef gas fill:#fff3b0,stroke:#f77f00,color:#5c3700;
    classDef process fill:#d8f3dc,stroke:#2d6a4f,color:#1b4332;
...
```mermaid
flowchart TB
    A[Air Lift Pump] --> B[Lift Tube]
    A --> C[Pipa Injeksi Udara]
    A --> D[Kompresor / Blower]
    A --> E[Diffuser]
    A --> F[Katup Kontrol Udara]
    A --> G[Outlet Air]

    B --> B1[Menentukan kapasitas aliran]
    C --> C1[Menyalurkan udara ke dasar pipa]
    D --> D1[Menyediakan tekanan dan debit udara]
    E --> E1[Membentuk gelembung kecil]
    F --> F1[Mengatur debit udara]
    G --> G1[Mengarahkan air keluar]

    classDef utama fill:#caf0f8,stroke:#0077b6,color:#03045e;
    classDef fungsi fill:#e9f5db,stroke:#588157,color:#1b4332;
    classDef kontrol fill:#ffe5d9,stroke:#e76f51,color:#6a040f;

    class A utama;
    class B,C,D,E,G fungsi;
    class F kontrol;
```
...
![konsep-diffuser](/static/images/agri/konsep-diffuser.webp)
...
```mermaid
flowchart TD
    A[Jenis Diffuser] --> B[Tube Diffuser]
    A --> C[Perforated Disc]

    B --> B1[Distribusi udara lebih merata]
    B --> B2[Gelembung lebih halus]
    B --> B3[Cocok untuk Air Lift Pump]

    C --> C1[Desain sederhana]
    C --> C2[Biaya lebih rendah]
    C --> C3[Efisiensi moderat]

    classDef pilihan fill:#d8f3dc,stroke:#2d6a4f,color:#1b4332;
    classDef ekonomis fill:#fff3b0,stroke:#f77f00,color:#5c3700;
    classDef pusat fill:#caf0f8,stroke:#0077b6,color:#03045e;

    class A pusat;
    class B,B1,B2,B3 pilihan;
    class C,C1,C2,C3 ekonomis;
```
...
```mermaid
flowchart TD
    A[Target kapasitas air<br/>1-5 m³/jam] --> B[Hitung luas penampang pipa<br/>Q = A × v]
    B --> C[Tentukan diameter lift tube]
    C --> D[Hitung tekanan hidrostatik<br/>P = ρ × g × h]
    D --> E[Tentukan tekanan kompresor]
    E --> F[Hitung rasio udara-air]
    F --> G[Tentukan kapasitas kompresor]
    G --> H[Validasi performa sistem]

    classDef input fill:#d9f0ff,stroke:#0077b6,color:#003049;
    classDef calc fill:#fff3b0,stroke:#f77f00,color:#5c3700;
    classDef output fill:#d8f3dc,stroke:#2d6a4f,color:#1b4332;
...
<figure>
  <img
    src="/static/images/agri/airlift-pump-design-5m3.png"
    alt="Airlift pump design 5 m3"
  />
  <figcaption>
    Desain airlift pump untuk sistem bioflok berkapasitas 5 m³ sebagai alat
    bantu sirkulasi air dan suplai oksigen.
  </figcaption>
</figure>
...
```mermaid
flowchart LR
    A[Input Desain] --> B[Debit Air<br/>1-5 m³/jam]
    A --> C[Kedalaman Air<br/>±2 m]
    A --> D[Material Pipa<br/>PVC / HDPE]

    B --> E[Diameter Lift Tube<br/>2-4,2 cm]
    C --> F[Tekanan Udara<br/>±0,3 bar]
    D --> G[Instalasi Pipa dan Diffuser]

    E --> H[Sistem Air Lift Pump]
    F --> H
    G --> H

    classDef input fill:#caf0f8,stroke:#0077b6,color:#03045e;
    classDef desain fill:#fff3b0,stroke:#f77f00,color:#5c3700;
    classDef hasil fill:#d8f3dc,stroke:#2d6a4f,color:#1b4332;
...
```mermaid
flowchart TD
    A[Air Lift Pump] --> B[Memindahkan Air]
    A --> C[Oksigenasi]
    A --> D[Degassing CO₂]
    A --> E[Perawatan Rendah]

    B --> F[Sirkulasi air lebih baik]
    C --> G[Oksigen terlarut meningkat]
    D --> H[Kualitas air lebih stabil]
    E --> I[Biaya operasional lebih rendah]

    classDef utama fill:#caf0f8,stroke:#0077b6,color:#03045e;
    classDef fungsi fill:#d8f3dc,stroke:#2d6a4f,color:#1b4332;
    classDef manfaat fill:#fff3b0,stroke:#f77f00,color:#5c3700;

    class A utama;
    class B,C,D,E fungsi;
...
```mermaid
flowchart TD
    A[Aplikasi Air Lift Pump] --> B[RAS / Akuakultur]
    A --> C[Hidroponik]
    A --> D[Pertanian Berkelanjutan]

    B --> B1[Sirkulasi air]
    B --> B2[Degassing CO₂]
    B --> B3[Oksigenasi ikan]

    C --> C1[Oksigenasi akar]
    C --> C2[Sirkulasi nutrisi]

    D --> D1[Hemat energi]
    D --> D2[Dapat dikombinasikan dengan panel surya]
    D --> D3[Mendukung sistem terpadu]

    classDef aplikasi fill:#caf0f8,stroke:#0077b6,color:#03045e;
    classDef akuakultur fill:#d8f3dc,stroke:#2d6a4f,color:#1b4332;
    classDef hidro fill:#fff3b0,stroke:#f77f00,color:#5c3700;
    classDef sustain fill:#ffe5d9,stroke:#e76f51,color:#6a040f;

    class A aplikasi;
    class B,B1,B2,B3 akuakultur;
    class C,C1,C2 hidro;
    class D,D1,D2,D3 sustain;
```
