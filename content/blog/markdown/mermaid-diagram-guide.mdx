...
- [Apendix Advanced: Bentuk Node pada Mermaid Diagram](#apendix-advanced-bentuk-node-pada-mermaid-diagram)
  - [A.1. Mengapa Bentuk Node Penting?](#a1-mengapa-bentuk-node-penting)
  - [A.2. Bentuk Node Dasar yang Sering Dipakai](#a2-bentuk-node-dasar-yang-sering-dipakai)
  - [A.3. Contoh Semua Bentuk Node Dasar](#a3-contoh-semua-bentuk-node-dasar)
  - [A.4. Node Lingkaran](#a4-node-lingkaran)
  - [A.5. Node Diamond untuk Decision Point](#a5-node-diamond-untuk-decision-point)
  - [A.6. Node Database untuk Log, Historian, dan Catatan Batch](#a6-node-database-untuk-log-historian-dan-catatan-batch)
  - [A.7. Node Segitiga untuk Warning atau Risiko](#a7-node-segitiga-untuk-warning-atau-risiko)
  - [A.8. Standar Makna Bentuk untuk Artikel Teknis](#a8-standar-makna-bentuk-untuk-artikel-teknis)
  - [A.9. Kombinasi Bentuk dan Warna](#a9-kombinasi-bentuk-dan-warna)
  - [A.10. Contoh Advanced untuk Pembiakan PGPM](#a10-contoh-advanced-untuk-pembiakan-pgpm)
  - [A.11. Catatan Kompatibilitas](#a11-catatan-kompatibilitas)
  - [A.12. Rekomendasi Pemakaian Advanced Shapes](#a12-rekomendasi-pemakaian-advanced-shapes)
  - [Tahap 1 — Bentuk Aman](#tahap-1--bentuk-aman)
  - [Tahap 2 — Bentuk Menengah](#tahap-2--bentuk-menengah)
  - [Tahap 3 — Bentuk Advanced](#tahap-3--bentuk-advanced)
  - [A.13. Kesimpulan Apendix](#a13-kesimpulan-apendix)
  - [B.1. Apa Itu Mermaid XY Chart?](#b1-apa-itu-mermaid-xy-chart)
  - [B.2. Struktur Dasar XY Chart](#b2-struktur-dasar-xy-chart)
  - [B.3. Contoh Line Chart: Spesiasi Karbonat vs pH](#b3-contoh-line-chart-spesiasi-karbonat-vs-ph)
  - [B.4. Versi yang Lebih Rapi untuk Warna Line Chart](#b4-versi-yang-lebih-rapi-untuk-warna-line-chart)
  - [B.5. Contoh Bar Chart](#b5-contoh-bar-chart)
  - [B.6. Contoh Kombinasi Bar dan Line](#b6-contoh-kombinasi-bar-dan-line)
  - [B.7. Kapan Mermaid Chart Cocok Digunakan?](#b7-kapan-mermaid-chart-cocok-digunakan)
  - [B.8. Rekomendasi Standar Penulisan Chart di Artikel](#b8-rekomendasi-standar-penulisan-chart-di-artikel)
  - [B.9. Kesimpulan Tambahan](#b9-kesimpulan-tambahan)
...
## B.1. Apa Itu Mermaid XY Chart?

Selain flowchart, sequence diagram, Gantt chart, dan mind map, Mermaid juga dapat membuat grafik data sederhana menggunakan `xychart` atau `xychart-beta`.

Format ini cocok untuk menampilkan hubungan antara dua variabel, misalnya:
...
hari → berat ikan
beban → arus motor
jam operasi → vibration trend
```

Contoh dasar:

````mdx
```mermaid id="contoh-xychart-01"
xychart-beta
    title "Contoh Line Chart"
    x-axis "Hari" [1, 2, 3, 4, 5]
    y-axis "Nilai" 0 --> 100
    line [10, 25, 45, 70, 90]
```
````
...
## B.2. Struktur Dasar XY Chart

Struktur umum:

````mdx
```mermaid id="nama-chart"
xychart-beta
    title "Judul Chart"
    x-axis "Nama Sumbu X" [x1, x2, x3, x4]
    y-axis "Nama Sumbu Y" min --> max
    line [y1, y2, y3, y4]
```
````
...
| Bagian         | Fungsi                                    |
| -------------- | ----------------------------------------- |
| `xychart-beta` | Menentukan jenis diagram sebagai XY chart |
| `title`        | Judul grafik                              |
| `x-axis`       | Label dan nilai sumbu-X                   |
| `y-axis`       | Label dan rentang sumbu-Y                 |
| `line`         | Data line chart                           |
| `bar`          | Data bar chart                            |

---

## B.3. Contoh Line Chart: Spesiasi Karbonat vs pH

Contoh yang Anda berikan sudah valid sebagai chart edukatif:
...
> Mermaid XY Chart belum nyaman untuk memberi nama langsung pada setiap garis seperti legend lengkap.

Karena itu, sebaiknya setelah chart diberi caption:

```mdx
**Gambar B.1.** Spesiasi karbonat terhadap pH. Garis pertama menunjukkan dominasi CO₂/H₂CO₃, garis kedua menunjukkan HCO₃⁻, dan garis ketiga menunjukkan CO₃²⁻.
```

---

## B.4. Versi yang Lebih Rapi untuk Warna Line Chart

Untuk `xychart`, pewarnaan line/bar yang lebih tepat dapat memakai `plotColorPalette` di dalam konfigurasi `xyChart`. Mermaid menyediakan theme variable khusus untuk XY Chart, termasuk `plotColorPalette` untuk warna bar atau line secara berurutan. ([Mermaid][1])

Contoh:

````mdx
```mermaid id="karbonat-ph-warna"
%%{init: {
  "theme": "base",
  "themeVariables": {
    "xyChart": {
      ...
      "titleColor": "#0D47A1",
      "xAxisLabelColor": "#263238",
      "xAxisTitleColor": "#0D47A1",
      "xAxisTickColor": "#546E7A",
      "xAxisLineColor": "#546E7A",
      "yAxisLabelColor": "#263238",
      "yAxisTitleColor": "#0D47A1",
      "yAxisTickColor": "#546E7A",
      "yAxisLineColor": "#546E7A",
      "plotColorPalette": "#1565C0, #2E7D32, #EF6C00"
    }
  }
}}%%
xychart-beta
...
**Gambar B.1.** Spesiasi karbonat terhadap pH. Garis biru mewakili CO₂/H₂CO₃, garis hijau mewakili HCO₃⁻, dan garis oranye mewakili CO₃²⁻.
````

---

## B.5. Contoh Bar Chart

Selain `line`, Mermaid XY Chart juga mendukung `bar`.

````mdx
```mermaid id="contoh-bar-chart-01"
xychart-beta
    title "Contoh Produksi Harian"
    x-axis "Hari" [1, 2, 3, 4, 5, 6, 7]
    y-axis "Produksi kg" 0 --> 100
    bar [40, 55, 60, 72, 68, 80, 90]
```
````
...
jumlah inspeksi
jumlah failure per kategori
```

---

## B.6. Contoh Kombinasi Bar dan Line

Mermaid XY Chart juga dapat menampilkan beberapa seri, misalnya bar dan line dalam satu chart.

````mdx
```mermaid id="contoh-bar-line-01"
xychart-beta
    title "Pakan Harian dan Pertumbuhan"
    x-axis "Hari" [1, 2, 3, 4, 5, 6, 7]
    y-axis "Nilai" 0 --> 100
    bar [20, 25, 30, 35, 40, 45, 50]
    line [10, 18, 28, 42, 55, 70, 85]
```
````

Untuk artikel budidaya, ini bisa digunakan sebagai ilustrasi:

```text
bar  = pakan harian
line = estimasi pertumbuhan biomassa
```

---

## B.7. Kapan Mermaid Chart Cocok Digunakan?

Gunakan Mermaid XY Chart untuk:

| Kebutuhan                         | Cocok?       |
| --------------------------------- | ------------ |
| Grafik edukatif sederhana         | Sangat cocok |
| Grafik tren ringan                | Cocok        |
| Ilustrasi hubungan parameter      | Cocok        |
| Grafik dengan sedikit data        | Cocok        |
| Chart untuk artikel MDX           | Cocok        |
| Grafik ilmiah kompleks            | Kurang cocok |
| Banyak seri dengan legend lengkap | Kurang cocok |
| Tooltip/interaktif                | Kurang cocok |
| Analisis statistik                | Tidak cocok  |

Untuk artikel teknis, Mermaid Chart sangat cocok sebagai **ilustrasi konsep**, bukan sebagai alat analisis data utama.

---

## B.8. Rekomendasi Standar Penulisan Chart di Artikel
...
xychart-beta
    title "Judul Grafik"
    x-axis "Nama Sumbu X" [x1, x2, x3]
    y-axis "Nama Sumbu Y" min --> max
    line [y1, y2, y3]
```

**Gambar X.** Keterangan grafik, termasuk arti setiap garis jika ada lebih dari satu seri.
````

Untuk multiple line, selalu tambahkan keterangan:

```mdx
**Keterangan garis:**

- Garis pertama = CO₂/H₂CO₃
- Garis kedua = HCO₃⁻
- Garis ketiga = CO₃²⁻
```

Karena tanpa legend, pembaca bisa sulit memahami arti masing-masing garis.

---

## B.9. Kesimpulan Tambahan

Mermaid ternyata tidak hanya untuk flowchart dan diagram proses, tetapi juga dapat membuat chart sederhana melalui `xychart-beta`. Fitur ini berguna untuk artikel MDX karena grafik dapat ditulis langsung sebagai teks, mudah direvisi, dan tidak membutuhkan file gambar terpisah.

Namun penggunaannya perlu proporsional. Untuk chart edukatif seperti pH vs spesiasi karbonat, Mermaid sudah cukup. Untuk grafik data aktual, dashboard, banyak seri, tooltip, atau analisis statistik, lebih tepat menggunakan Recharts, Vega-Lite, Python/Matplotlib, atau Excel.

[1]: https://mermaid.ai/open-source/syntax/xyChart.html 'XY Chart | Mermaid'
