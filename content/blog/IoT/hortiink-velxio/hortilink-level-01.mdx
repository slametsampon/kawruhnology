...
### 7.1A Diagram Layering dan Class Mapping

Agar pembaca tidak hanya membaca empat layer firmware sebagai daftar teks, diagram berikut memperlihatkan bagaimana layer dan class dipetakan dalam struktur HortiLink.
...
flowchart TB
    ...
    subgraph SYS["sys"]
      direction TB
      SYS1["Config<br/>pin map<br/>timing<br/>identity<br/>default constants"]
    end

    subgraph DRV["drv"]
      direction TB
      DRV1["StatusLed<br/>hardware boundary"]
      DRV2["Button / Sensor / Relay<br/>hardware driver"]
    end

    subgraph SVC["svc"]
      direction TB
      SVC1["Mode Logic Service"]
      SVC2["Rule / Alarm / Command Service"]
    end

    subgraph APP["app"]
      direction TB
      APP1["FirmwareApp<br/>runtime orchestration"]
    end
...
### 7.1B Contoh Nyata pada Code Level 1

Pada Level 1, mapping layer itu sudah tampak jelas di level class. Diagram berikut menggunakan class yang benar-benar ada pada sketch Level 1.
...
flowchart TB
    ...
    subgraph SVC["svc"]
      direction TB
      S["svc::NodeIdentityService"]
      M["NodeMode<br/>BOOTING / LOCAL_ONLY / GATEWAY_LINKED / ALERT"]
    end
...
Pada sketch Level 1, `NodeIdentityService` memang memakai `drv::StatusLed`, lalu `FirmwareApp` hanya memakai `svc::NodeIdentityService`, bukan langsung memegang driver. Itu terlihat jelas pada wiring object `drv_statusLed`, `svc_nodeIdentityService`, dan `app_firmware`. :contentReference[oaicite:3]{index=3} :contentReference[oaicite:4]{index=4}

---

### 7.1C Diagram Object Wiring

Selain class, pembaca juga perlu melihat bagaimana object dirangkai pada composition root. Diagram ini tidak lagi menunjukkan “jenis class”, tetapi object nyata yang dibuat di sketch.
...
Itu tepat dengan composition root Level 1 yang membuat `drv::StatusLed`, lalu `svc::NodeIdentityService(drv_statusLed)`, lalu `app::FirmwareApp(svc_nodeIdentityService)`. :contentReference[oaicite:5]{index=5}

---

### 7.1D Diagram Aliran Data dan Perintah

Setelah struktur layer dan class jelas, pembaca juga perlu melihat aliran data dan perintah secara ringkas. Diagram berikut sengaja dibuat sederhana agar tidak mencampur arsitektur dengan detail runtime mikro.
...
flowchart TB
    ...
    IN["Input / waktu / event"]
    DRV["drv<br/>hardware read / write"]
    SVC["svc<br/>decision & domain logic"]
    APP["app<br/>runtime orchestration"]
    OUT["LED / actuator / log"]
    ...
    classDef in fill:#FFF8E1,stroke:#F9A825,color:#111,stroke-width:1.4px;
    classDef drv fill:#E8F5E9,stroke:#2E7D32,color:#111,stroke-width:1.6px;
    classDef svc fill:#FFF3E0,stroke:#EF6C00,color:#111,stroke-width:1.6px;
    classDef app fill:#FCE4EC,stroke:#C2185B,color:#111,stroke-width:1.6px;
    classDef out fill:#E0F7FA,stroke:#00838F,color:#111,stroke-width:1.6px;
