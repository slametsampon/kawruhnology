...
- [**_README: HortiLink — Aliran Belajar_**](#readme-hortilink--aliran-belajar)
  - [Bagian 1 — Orientasi Sistem HortiLink](#bagian-1--orientasi-sistem-hortilink)
    ...
    - [1.2A Opsi Topologi Komunikasi Multi-Node](#12a-opsi-topologi-komunikasi-multi-node)
    - [1.2B Channel Komunikasi Operasional](#12b-channel-komunikasi-operasional)
  ...
  - [Bagian 4 — Feature System HortiLink dari Sudut Pandang Operasional](#bagian-4--feature-system-hortilink-dari-sudut-pandang-operasional)
    ...
    - [4.3A Brokered Node-to-Node Interaction](#43a-brokered-node-to-node-interaction)
  ...
  - [Bagian 5 — Mode Operasi Resmi HortiLink](#bagian-5--mode-operasi-resmi-hortilink)
    ...
    - [5.3 Prioritas Keputusan Runtime](#53-prioritas-keputusan-runtime)
  ...
  - [Bagian 7 — Urutan Implementasi Belajar yang Natural](#bagian-7--urutan-implementasi-belajar-yang-natural)
    ...
    - [7.3 Batas Nyaman Jumlah Node per Topologi](#73-batas-nyaman-jumlah-node-per-topologi)
  - [Bagian 8 — Definisi Formal HortiLink v0.1](#bagian-8--definisi-formal-hortilink-v01)
    ...
    - [8.6A Roadmap Pertumbuhan Arsitektur](#86a-roadmap-pertumbuhan-arsitektur)
...
### 1.2A Opsi Topologi Komunikasi Multi-Node

Saat HortiLink berkembang dari satu node menjadi banyak node, bentuk komunikasi antar node menjadi keputusan arsitektur yang sangat penting. Secara umum, ada tiga opsi yang bisa dipakai:

- **Full star via Raspberry Pi**
- **Hybrid star + limited peer**
- **Full peer-to-peer**

Untuk HortiLink, ketiganya perlu dikenali sejak awal agar pembaca memahami kenapa Raspberry Pi kemudian diposisikan sebagai pusat koordinasi lokal.
...
flowchart TB
    ...
    classDef star fill:#E8F5E9,stroke:#2D6A4F,color:#1B4332,stroke-width:2px;
    ...
    classDef pi fill:#DBEAFE,stroke:#2563EB,color:#1E3A8A,stroke-width:2px;
...
### 1.2B Channel Komunikasi Operasional

Dalam HortiLink, tidak semua jalur komunikasi mempunyai peran yang sama. Smartphone, node ESP32, dan Raspberry Pi harus dibaca sebagai tiga channel yang berbeda fungsinya:

- **Smartphone → Node** = operasi lokal oleh manusia
- **Node → Raspberry Pi** = komunikasi machine-to-machine
- **Node ↔ Node** = secara logika mungkin terjadi, tetapi transport utamanya tetap dibrokerkan melalui Raspberry Pi

Dengan pemisahan ini, sistem menjadi lebih mudah dipahami dan lebih sehat secara SoC.
...
[**Kembali ke Atas**](#readme-hortilink--aliran-belajar)
...
### 4.3A Brokered Node-to-Node Interaction

Saat node mulai bertambah, kebutuhan interaksi antar node akan muncul. Tetapi interaksi itu tidak sebaiknya diterjemahkan sebagai semua node saling terhubung langsung. Untuk HortiLink, pola yang lebih sehat adalah:

> **logical node-to-node interaction, physical brokered interaction**

Artinya, secara fungsi Node A bisa mempengaruhi Node B, tetapi transport utamanya dilakukan melalui Raspberry Pi sebagai broker/gateway lokal.
...
### 5.3 Prioritas Keputusan Runtime

Saat sistem mulai menerima banyak sumber pengaruh — sensor, tombol lokal, smartphone, gateway, atau peer node — maka HortiLink harus punya aturan prioritas yang jelas. Tanpa urutan prioritas, sistem akan mudah bentrok dan sulit dipelihara.

Urutan prioritas yang sehat adalah:
...
### 6.4A Diagram Mapping Layer Firmware

Agar pembaca tidak hanya membaca layer sebagai daftar teks, diagram berikut memperlihatkan bagaimana empat layer firmware HortiLink diposisikan secara arsitektural.
...
flowchart TB
    SYS["sys<br/>config & identity"]
    DRV["drv<br/>hardware boundary"]
    SVC["svc<br/>domain logic"]
    APP["app<br/>runtime orchestration"]
...
### 6.5A Diagram Aliran Data, Keputusan, dan Perintah

Selain struktur layer, pembaca juga perlu melihat bagaimana aliran data dan perintah berjalan di dalam arsitektur firmware HortiLink. Diagram berikut memperlihatkan aliran yang sehat mulai dari input fisik, diproses menjadi keputusan domain, lalu diterapkan kembali ke output fisik dengan ritme runtime yang diatur oleh `app`.
...
flowchart TB
    HWIN["Input fisik<br/>sensor / button"]
    DRV["drv<br/>read / write device"]
    SVC["svc<br/>rule, mode, decision"]
    APP["app<br/>schedule & orchestration"]
    HWOUT["Output fisik<br/>LED / relay / actuator"]
    LOG["Serial / MQTT / UI"]
    ...
    classDef drv fill:#DBEAFE,stroke:#2563EB,color:#1E3A8A,stroke-width:2px;
    classDef svc fill:#DCFCE7,stroke:#16A34A,color:#14532D,stroke-width:2px;
    classDef app fill:#F3E8FF,stroke:#7C3AED,color:#4C1D95,stroke-width:2px;
    classDef out fill:#FCE7F3,stroke:#DB2777,color:#831843,stroke-width:2px;
    classDef log fill:#F1F5F9,stroke:#64748B,color:#334155,stroke-width:2px;
...
### 6.5B Contoh Mapping Feature ke Layer

Agar mapping tidak berhenti di level teori, berikut contoh visual bagaimana feature HortiLink ditempatkan ke layer firmware.
...
flowchart TB
    F1["Node identity"]
    F2["Manual button"]
    F3["Sensor acquisition"]
    F4["Auto rule"]
    F5["Alarm / fault"]
    F6["Runtime scheduling"]
    ...
    classDef feat fill:#FEFCE8,stroke:#CA8A04,color:#713F12,stroke-width:2px;
    classDef sys fill:#F1F5F9,stroke:#64748B,color:#334155,stroke-width:2px;
    classDef drv fill:#DBEAFE,stroke:#2563EB,color:#1E3A8A,stroke-width:2px;
    classDef svc fill:#DCFCE7,stroke:#16A34A,color:#14532D,stroke-width:2px;
    classDef app fill:#F3E8FF,stroke:#7C3AED,color:#4C1D95,stroke-width:2px;
...
### 7.3 Batas Nyaman Jumlah Node per Topologi

Agar pembaca tidak hanya melihat arsitektur secara teoritis, perlu diberikan gambaran kasar tentang skala node yang masih nyaman untuk tiap topologi. Angka berikut bukan batas keras, tetapi **rule-of-thumb engineering** untuk sistem lokal seperti HortiLink.
...
<ResourceBox
  ...
  items={[
    {
      name: 'HortiLink Level 1 — Node Identity and Status',
      href: '/blog/IoT/hortiink-velxio/hortilink-level-01',
    },
  ]}
/>
...
<ResourceBox
  ...
  items={[
    {
      name: 'HortiLink Level 2 — Local Interaction',
      href: '/blog/IoT/hortiink-velxio/hortilink-level-02',
    },
  ]}
/>
...
<ResourceBox
  ...
  items={[
    {
      name: 'HortiLink Level 3 — Sensor and Rule',
      href: '/blog/IoT/hortiink-velxio/hortilink-level-03',
    },
  ]}
/>
...
<ResourceBox
  ...
  items={[
    {
      name: 'HortiLink Level 4 — Runtime Discipline',
      href: '/blog/IoT/hortiink-velxio/hortilink-level-04',
    },
    {
      name: 'HortiLink Level 4A — Local HTTP Access',
      href: '/blog/IoT/hortiink-velxio/hortilink-level-04A',
    },
  ]}
/>
...
### 8.6A Roadmap Pertumbuhan Arsitektur

Agar pertumbuhan HortiLink tetap mudah dipahami, pembaca juga perlu melihat bagaimana roadmap arsitektur berkembang dari akses lokal satu node menuju koordinasi multi-node yang lebih matang.
