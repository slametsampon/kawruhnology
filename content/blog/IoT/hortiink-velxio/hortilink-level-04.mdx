...
Agar pembaca tidak hanya membaca sketch sebagai kumpulan kode, diagram berikut memperlihatkan bagaimana class pada Level 4 dipetakan ke layer firmware HortiLink.
...
flowchart TB
    ...
    subgraph SYS["namespace sys"]
      direction TB
      C["sys::Config<br/>pin, timing, defaults, LED pattern"]
    end
    ...
    subgraph SVC["namespace svc"]
      ...
      S1["NodeMode"]
      S2["RuntimeEvent"]
      S3["PersistentLocalConfig"]
      S4["HealthSnapshot"]
      S5["svc::RuntimeDisciplineService"]
      S6["svc_persistentShadowStore"]
    end
...
Selain class, pembaca juga perlu melihat bagaimana object nyata dirangkai pada composition root. Diagram ini menunjukkan urutan dependency yang benar pada Level 4.
...
flowchart TB
    ...
    O2["drv_soilSensor"]
    O3["drv_actuator"]
    O4["svc_runtimeDisciplineService"]
    O5["app_firmware"]
...
### 9.3 Diagram Struktur Domain di dalam `svc`

Karena Level 4 sudah lebih kompleks dibanding Level 3, pembaca juga perlu melihat struktur domain internal pada layer `svc`.
...
flowchart TB
    ...
    M["NodeMode"]
    E["RuntimeEvent"]
    P["PersistentLocalConfig"]
    H["HealthSnapshot"]
    SH["svc_persistentShadowStore"]
    R["RuntimeDisciplineService"]
    ...
    class M,E,P,H,SH state;
    class R svc;
```

---

### 9.4 Diagram Interaksi Runtime Ringkas

Setelah struktur layer, object, dan domain `svc` jelas, diagram berikut memperlihatkan interaksi runtime Level 4 secara ringkas tanpa mencampur terlalu banyak detail implementasi.
...
flowchart TB
    ...
    T["millis() / time"]
    C1["app::FirmwareApp<br/>control scheduler"]
    C2["app::FirmwareApp<br/>health scheduler"]
    S["svc::RuntimeDisciplineService"]
    D1["drv::DigitalSoilSensor<br/>update + read"]
    D2["drv::StatusLed<br/>set state"]
    D3["drv::ActuatorOutput<br/>set state"]
    L["app::FirmwareApp<br/>event + health log"]
