...
Fokus modul ini adalah memastikan bahwa **event asinkron** dapat diproses tanpa merusak kontrak komponen, tanpa menambah kompleksitas di `main.ino`, dan tanpa mengorbankan stabilitas sistem. Dengan demikian, interrupt menjadi bagian yang terkontrol dalam arsitektur firmware modular, bukan pengecualian yang merusak desain.
...
> `main.ino` (FINAL, UTUH, SIAP COPAS)
...
5. **main.ino tidak mengandung logic hardware tombol/LED**

   - Tidak ada `digitalWrite`, tidak ada `attachInterrupt` di `main.ino`.
...
- `main.ino` tetap berperan sebagai orchestrator, bukan tempat logika event
