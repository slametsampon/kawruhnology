...
- Menghubungkan actuator dengan **data sensor dan event** tanpa menambah kompleksitas di `main.ino`
...
void RelayActuator::update() {
  ...
  // Keputusan aktivasi dilakukan di logic aplikasi (mis. main.ino) berdasarkan data sensor
}
...
2. **Tidak ada logika hardware actuator di `main.ino`**
...
3. **Logika aktif HIGH/LOW terenkapsulasi**
   ...
   - Mengubah active level cukup ganti:
     ...
     tanpa ubah `main.ino` logic kontrol
...
- **Tidak ada logika hardware di `main.ino`**
  Tidak terdapat `pinMode`, `digitalWrite`, atau detail aktif HIGH/LOW di `main.ino`. Seluruhnya terenkapsulasi di class actuator.
