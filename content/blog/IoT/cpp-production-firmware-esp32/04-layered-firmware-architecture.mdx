...
- [📘 Artikel 4: Firmware Architecture Berbasis Class (Layered \& Deterministic)](#-artikel-4-firmware-architecture-berbasis-class-layered--deterministic)
  ...
  - [3. Design Principle (Rule yang Dikunci)](#3-design-principle-rule-yang-dikunci)
    ...
    - [🔒 Rule 4 — Service is Domain Boundary (Final)](#-rule-4--service-is-domain-boundary-final)
    - [🔒 Rule 5 — Application is System Decision Layer (Final)](#-rule-5--application-is-system-decision-layer-final)
    - [🔒 Rule 6 — Composition Root Only in IndustrialNode.ino](#-rule-6--composition-root-only-in-industrialnodeino)
    - [Setelah Artikel 4](#setelah-artikel-4)
    - [Inti yang Harus Dipahami Engineer Baru](#inti-yang-harus-dipahami-engineer-baru)
    ...
    - [🔒 Rule 6 — Composition Root Only in IndustrialNode.ino](#-rule-6--composition-root-only-in-industrialnodeino-1)
    - [Setelah Artikel 4](#setelah-artikel-4-1)
    - [Inti yang Harus Dipahami Engineer Baru](#inti-yang-harus-dipahami-engineer-baru-1)
...
Tujuannya bukan “rapi secara teori”, tetapi:
...
```text
app_   → logic utama sistem
svc_   → pengelola domain
drv_   → akses hardware
sys_   → tipe dan konfigurasi dasar
```
...
- driver memanggil service → driver tidak bisa dipakai ulang
- service memanggil app → logika jadi berputar
- circular include → compile error atau coupling tinggi
...
### 🔒 Rule 4 — Service is Domain Boundary (Final)
...
### 🔒 Rule 5 — Application is System Decision Layer (Final)
...
Layering ini bukan untuk “rapi”, tetapi untuk:
...
- ubah driver → tidak merusak application
- ubah logic → tidak menyentuh hardware
- ganti sensor → hanya ubah driver/service
...
- jika suhu > threshold → nyalakan relay
- jika error → masuk fail-safe
