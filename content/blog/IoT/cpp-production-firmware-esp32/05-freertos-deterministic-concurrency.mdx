...
- [📘 Artikel 5: OOP + FreeRTOS Tanpa Membunuh Determinism](#-artikel-5-oop--freertos-tanpa-membunuh-determinism)
  ...
  - [4. Implementation Pattern (ESP32 Arduino Context)](#4-implementation-pattern-esp32-arduino-context)
    ...
    - [4.2 ISR → Queue / Notify Pattern](#42-isr--queue--notify-pattern)
    - [4.3 Queue Communication Pattern](#43-queue-communication-pattern)
    - [4.4 Kenapa Harus Queue (Bukan Shared State)](#44--kenapa-harus-queue-bukan-shared-state)
    - [4.5 Kenapa Harus Fixed Size \& Bounded](#45-kenapa-harus-fixed-size--bounded)
    - [4.6 Hubungan dengan Layer](#46-hubungan-dengan-layer)
    - [4.7 Ringkasan Implementation Pattern](#47-ringkasan-implementation-pattern)
...
Struktur ini mengatur **siapa boleh memanggil siapa**,
tetapi belum mengatur **siapa menjalankan kode tersebut**.
...
Struktur ini mengatur **alur pemanggilan fungsi**,
tetapi tidak mengatur **konteks eksekusi**.
...
Masalah muncul bukan karena ada banyak task,
tetapi karena:
...
`ControlTask` adalah representasi dari **unit eksekusi (task)**
yang bertanggung jawab menjalankan application.
...
- `ControlApp` → decision layer (app\_)
